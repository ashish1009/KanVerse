...
namespace KanVest
{
  ...
  bool StockManager::UpdateStockInternal(const std::string& symbolName, bool forceRefresh)
  {
    try
    {
      ...
      
      {
        ...
        if (it != s_cache.end() && it->second.IsValid() && !forceRefresh)
        {
      ...
      StockData newData = FetchAndParse(symbol, key.interval, key.range);
  ...
  void StockManager::RefreshAll()
  {
    ...
    {
      s_threadPool->Enqueue([sym]() { UpdateStockInternal(sym, false); });
    }
  }
  ...
  void StockManager::RefreshAllBlocking()
  {
    ...
    for (const auto& sym : symbolsCopy)
    {
      futs.push_back(std::async(std::launch::async, [sym]() { UpdateStockInternal(sym, false); }));
    }
  ...
  {
    std::scoped_lock lock(s_mutex);
    s_selectedStockSymbol = NormalizeSymbol(stockSymbol);
  ...
  void StockManager::UpdateLoop(int intervalMilliseconds)
  {
    ...
    while (s_running)
    {
      ...
      if (sleepFor > 0ms) std::this_thread::sleep_for(sleepFor);
    }
  }
