...
namespace KanVest
{
  MACDResult MACD::Compute(const StockData& stock)
  {
    ...
    // Compute EMAs
    auto ema12 = MovingAverage::ComputeEMA(closes, 12);
    auto ema26 = MovingAverage::ComputeEMA(closes, 26);
    ...
    result.signalLine = MovingAverage::ComputeEMA(result.macdLine, 9);
