...
#define Font(font) KanVest::UI::Font::Get(KanVest::UI::FontType::font)
  ...
  void UI_MovingAverage::ShowDMA(const StockData& stockData, ImTextureID shadowTexture)
  {
    ...
    for (const auto& [color, summary] : analyzerResult.summary.at(TechnicalIndicators::DMA))
    {
  ...
  void UI_MovingAverage::ShowEMA(const StockData& stockData, ImTextureID shadowTexture)
  {
    ...
    const auto& analyzerResult = Analyzer::GetReport();
    for (const auto& [color, summary] : analyzerResult.summary.at(TechnicalIndicators::EMA))
    {
      KanVasX::UI::Text(Font(Regular), summary, Align::Left, {0, 0}, color);
    }
  }
