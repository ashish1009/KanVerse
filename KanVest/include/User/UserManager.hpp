...
namespace KanVest
{
  ...
  {
  public:
    /// This function saves user data infile
    /// - Parameters:
    ///   - map: user data map
    ///   - filePath: file path
    static bool SaveToYAML(const std::unordered_map<std::string, User>& map, const std::string& filePath);
    /// This function load user data from file
    /// - Parameters:
    ///   - map: user data map
    ///   - filePath: file path
    static bool LoadFromYAML(std::unordered_map<std::string, User>& map, const std::string& filePath);
  };
